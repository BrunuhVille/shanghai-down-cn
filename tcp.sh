...
installxanmod(){
	...
	if [[ "${release}" == "centos" ]]; then
		...
		if [[ ${bit} = "x86_64" ]]; then
			wget -N -O linux-headers-d10.deb https://chinagz2018-my.sharepoint.com/:u:/g/personal/ylx_chinagz2018_onmicrosoft_com/EfL2P-ySGcdFm1NpYtv0WB4Bfmzox5ljxiOcre5ScYH5mw?download=1
			wget -N -O linux-image-d10.deb https://chinagz2018-my.sharepoint.com/:u:/g/personal/ylx_chinagz2018_onmicrosoft_com/ES3kPY9OZk1PoMen1vqW_XwBmb92d4LELyAkl-2Wh9KWxA?download=1
				
			dpkg -i linux-image-d10.deb
			dpkg -i linux-headers-d10.deb
				
			kernel_version="5.8.1-xanmod1"
		else
			echo -e "${Error} 还在用32位内核，别再见了 !" && exit 1	
		fi		
	fi
...
————————————内核管理————————————
 ${Green_font_prefix}1.${Font_color_suffix} 安装 BBR原版内核 - 5.6.15/5.7.7/5.8.1
 ${Green_font_prefix}2.${Font_color_suffix} 安装 BBRplus版内核 - 4.14.129
 ${Green_font_prefix}3.${Font_color_suffix} 安装 Lotserver(锐速)内核 - 多种
 ${Green_font_prefix}4.${Font_color_suffix} 安装 xanmod版内核 - 5.5.1/5.7.7/5.8.1
