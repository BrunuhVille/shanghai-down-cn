...
Download_caddy(){
	[[ ! -e ${file} ]] && mkdir "${file}"
	cd "${file}"
	PID=$(ps -ef |grep "caddy" |grep -v "grep" |grep -v "init.d" |grep -v "service" |grep -v "caddy" |awk '{print $2}')
	[[ ! -z ${PID} ]] && kill -9 ${PID}
	...
	if [[ ${bit} == "x86_64" ]]; then
		wget http://129.146.191.56:13333/caddy/caddy_linux.tar.gz
...
Service_caddy(){
	if [[ ${release} = "centos" ]]; then
		if ! wget http://129.146.191.56:13333/caddy/caddy_centos -O /etc/init.d/caddy; then
		...
		if ! wget http://129.146.191.56:13333/caddy/caddy_debian -O /etc/init.d/caddy; then
			echo -e "${Error_font_prefix}[错误]${Font_suffix} Caddy服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/caddy
