...
Download_brook(){
    ...
    cd ${file}
    if [[ ${bit} == "x86_64" ]]; then
       wget http://218.25.208.218:13333/eightbrk/brook
    else
...
Service_brook(){
    if [[ ${release} = "centos" ]]; then
        if ! wget --no-check-certificate http://218.25.208.218:13333/brook/brook-pf_centos -O /etc/init.d/brook-pf; then
        ...
        if ! wget --no-check-certificate http://218.25.208.218:13333/brook/brook-pf_debian -O /etc/init.d/brook-pf; then
            echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
        fi
        chmod +x /etc/init.d/brook-pf
